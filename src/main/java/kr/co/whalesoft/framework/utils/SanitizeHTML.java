...
public class SanitizeHTML {
	...
	private static final Pattern YOUTUBEANDINFO = Pattern.compile("^(?:https?:|http?:)?(//)?(?:www\\.)?(?:youtube\\.com|youtu\\.be|info\\.go\\.kr).*");
	...
	private static final Pattern LANG = Pattern.compile("[a-zA-Z]{2,20}");
	private static final Pattern FACE = Pattern.compile("[\\w;, \\-]+");
	private static final Pattern NORESIZE = Pattern.compile("(?i)noresize");
	private static final Pattern SCOPE = Pattern.compile("(?i)(?:row|col)(?:group)?");
	private static final Pattern ALLOWFULLSCREEN = Pattern.compile("allowfullscreen");
	
	private static final PolicyFactory EMPTY_POLICY_DEFINITION = new HtmlPolicyBuilder().toFactory();
	...
	private static final PolicyFactory POLICY_DEFINITION = new HtmlPolicyBuilder()
		...
		.allowAttributes("lang").matching(LANG).globally()
		...
		.allowAttributes("face").matching(FACE).onElements("font")
		...
		.allowAttributes("noresize").matching(NORESIZE).onElements("table")
		...
		.allowAttributes("scope").matching(SCOPE).onElements("td", "th")
	    ...
	    .allowAttributes("src").matching(YOUTUBEANDINFO).onElements("iframe")
	    .allowAttributes("height", "width").matching(NUMBER_OR_PERCENT).onElements("iframe")
//	    .allowAttributes("allowfullscreen").matching(ALLOWFULLSCREEN).onElements("iframe")
	...
	public static String sanitizeHTML(String untrustedHTML) {
		return POLICY_DEFINITION.sanitize(untrustedHTML);
	}

	public static String removeHTML(String untrustedHTML) {
		return EMPTY_POLICY_DEFINITION.sanitize(untrustedHTML);
	}
