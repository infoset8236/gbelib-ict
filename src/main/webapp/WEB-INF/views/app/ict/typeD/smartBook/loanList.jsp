...
<c:if test="${fn:length(loanList.dsMyLibraryList) < 1 }">
    <div class="exception" style="margin-top: 30rem">대출중인 도서 내역이 없습니다.</div>
</c:if>
            ...
            <li class="resultItem">
                <div class="bookInfo">
                    <div class="bookDetail">
                        ...
                        <div class="bookMeta">
                            <div tabIndex="0" class="dataRoom">대출일<span><fmt:parseDate var="curDate" value="${i.LOAN_DATE}" pattern="yyyyMMdd"/>
			<fmt:formatDate value="${curDate}" type="both" pattern="yyyy-MM-dd"/></span></div>
                            <div tabIndex="0" class="loanDate">반납예정일<span><fmt:parseDate var="curDate" value="${i.RETURN_PLAN_DATE}" pattern="yyyyMMdd"/>
			<fmt:formatDate value="${curDate}" type="both" pattern="yyyy-MM-dd"/></span></div>
                            <div tabIndex="0" class="libraryName">소장처명<span>${i.LOAN_LOCA_NAME}</span></div>
                            <div tabIndex="0" class="returnDate">자료실명<span>${i.RETURN_LOCA_NAME}</div>
                        </div>
                    </div>
                </div>
