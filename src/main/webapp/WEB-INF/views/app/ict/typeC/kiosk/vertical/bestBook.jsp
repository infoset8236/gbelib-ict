...
<html lang="ko">
	...
	<div class="kiosk_best_book_container">
		<div class="kiosk_common_header_wrapper">
			<div class="kiosk_common_header_kr">대출베스트</div>
			<div class="kiosk_common_header_en">BEST BOOK</div>
		</div>
		<div class="kiosk_best_book_swiper">
			<div class="swiper">
				<div class="swiper-wrapper">
					<!-- 슬라이드 아이템 -->
					<c:forEach items="${bestBookList.dsLoanBestList}" var="i" varStatus="status">
					<div class="swiper-slide">
						<div class="swiper-item" onclick="goDetail('${i.LOCA}','${i.CTRLNO}');">
							<c:choose>
							...
							<div class="kiosk_best_book_badge">${i.RANK}위</div>
							<div>
								<div class="kiosk_best_book_title">${i.TITLE}</div>
								<div class="kiosk_best_book_writer">${i.AUTHOR}</div>
								<div class="kiosk_best_book_year">${i.PUBLER}ㆍ${i.PUBLER_YEAR}</div>
								<div class="kiosk_best_book_info"><span>소장위치</span>${i.SUB_LOCA_NAME}</div>
								<div class="kiosk_best_book_info"><span>청구기호</span>${i.CALL_NO}</div>
							</div>
						</div>
					</div>
					</c:forEach>
