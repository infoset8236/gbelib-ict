...
<html lang="ko">
	...
	<form:form modelAttribute="teach" id="teach" action="teach.do" method="GET">
		...
		<div class="kiosk_course_information_container">
			<div class="kiosk_common_header_wrapper">
				<div class="kiosk_common_header_kr">강좌안내</div>
				<div class="kiosk_common_header_en">COURSE INFORMATION</div>
			</div>
			<div class="kiosk_course_information_swiper">
								...
								<div class="swiper-item">
									<c:choose>
										<c:when test="${i.teach_status eq '0'}">
											<div class="kiosk_course_information_badge status_${i.teach_status}">수강신청</div>
										</c:when>
										...
										<c:when test="${i.teach_status eq '2' or i.teach_status eq '10'}">
											<div class="kiosk_course_information_badge status_${i.teach_status}">신청완료</div>
										</c:when>
										<c:when test="${i.teach_status eq '3'}">
											<div class="kiosk_course_information_badge status_${i.teach_status}">대기자 신청완료</div>
										</c:when>
										<c:when test="${i.teach_status eq '9'}">
											<div class="kiosk_course_information_badge status_${i.teach_status}">수강종료</div>
										</c:when>
										<c:when test="${i.teach_status eq '4'or i.teach_status eq '44'}">
											<div class="kiosk_course_information_badge status_${i.teach_status}">접수마감</div>
										</c:when>
										<c:when test="${i.teach_status eq '5'}">
											<div class="kiosk_course_information_badge status_${i.teach_status}">정원마감</div>
										</c:when>
										<c:when test="${i.teach_status eq '6'}">
											<div class="kiosk_course_information_badge status_${i.teach_status}">신청대기</div>
										</c:when>
									</c:choose>
									<div>
										<div class="kiosk_course_information_title">${i.teach_name}</div>
										<div class="kiosk_course_information_caption"><span>접수기간</span>${i.start_join_date} ~ ${i.end_join_date}</div>
										<div class="kiosk_course_information_caption"><span>수강대상</span>${i.teach_target}</div>
									</div>
									<div class="kiosk_course_information_counter_wrapper">
										<div class="kiosk_course_information_counter_top">
											<div class="kiosk_course_information_counter_label">모집인원</div>
											<div class="kiosk_course_information_counter_value"><span class="highlight">${i.teach_join_count}</span><span class="dash">/</span>${i.teach_limit_count}</div>
										</div>
										<div class="kiosk_course_information_counter_bottom">
											<div class="kiosk_course_information_counter_label">후보인원</div>
											<div class="kiosk_course_information_counter_value"><span class="highlight">${i.teach_backup_join_count}</span><span class="dash">/</span>${i.teach_backup_count}</div>
										</div>
									</div>
								</div>
							</div>
